    ...
    <div className="flex flex-1 items-center gap-2 px-3 py-2 justify-end">
      ...
      {error && !setupRequired ? (
        ...
        <div className="flex items-center gap-2 animate-pulse">
          ...
          <span className="text-xs font-medium">Генерирую ответ…</span>
        </div>
      ) : isProcessing ? (
        <div className="flex items-center gap-2 animate-pulse">
          <LoaderIcon className="w-4 h-4 animate-spin" />
          <span className="text-xs font-medium">Распознаю…</span>
        </div>
      ) : capturing ? (
        <div className="flex items-center gap-2 text-green-600 animate-pulse">
          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
          <span className="text-xs font-medium">Слушаю…</span>
        </div>
      ) : null}
    </div>
  );
